...
class GCNPolicy(object):
    ...
    def _init(self, ob_space, ac_space, kind, atom_type_num, char_type_num, args):
        ...
        if args.has_attention == 1:
            ...
            if args.is_train == 1:
                ...
                sample = cond_sample
        sample = tf.expand_dims(sample, axis=2)  # b*1*1*v
        trans_sample = tf.transpose(sample, [0, 1, 3, 2])
        ob_node = tf.concat([ob['node'], trans_sample], axis=-1)
        # ob_node = GCN_cond_batch(ob['adj'], ob['node'], args.smi_importance * trans_sample, args.emb_size, name='fusion_1')
        # ob_node = GCN_cond_batch(ob['adj'], ob_node, args.smi_importance * trans_sample, args.emb_size,
        #                          name='fusion_2')  # tf.concat([node, args.smi_importance * trans_sample], axis=-1)
        # ob_node = GCN_cond_batch(ob['adj'], ob_node, args.smi_importance * trans_sample, args.emb_size, name='fusion_3')
        if args.bn == 1:
            ob_node = tf.layers.batch_normalization(ob_node, axis=-1)
        ob_node = tf.layers.dense(ob_node, 8, activation=None, use_bias=False, name='emb_2')
        # ob_node = GCN_batch(ob['adj'], ob_node, args.emb_size, name='fusion_gcn1', aggregate=args.gcn_aggregate)
        # ob_node = GCN_batch(ob['adj'], ob_node, args.emb_size, name='fusion_gcn2', aggregate=args.gcn_aggregate)
        # ob_node = GCN_batch(ob['adj'], ob_node, args.emb_size, name='fusion_gcn3', aggregate=args.gcn_aggregate)
        if args.bn == 1:
            ob_node = tf.layers.batch_normalization(ob_node, axis=-1)
        ...
        for i in range(args.layer_num_g-2):
            ...
            if args.bn == 1:
                emb_node = tf.layers.batch_normalization(emb_node, axis=-1)
        emb_node = GCN_batch(ob['adj'], emb_node, args.emb_size, is_act=False, is_normalize=(args.bn == 0), name='gcn2', aggregate=args.gcn_aggregate)
        emb_node = tf.squeeze(emb_node, axis=1)  # B*1*n*f -> B*n*f
    ...
    #
    #         ob_len = tf.reduce_sum(tf.squeeze(tf.cast(tf.cast(tf.reduce_sum(node, axis=-1), dtype=tf.bool), dtype=tf.float32), axis=-2), axis=-1)  # B, each row is the number of graph actual nodes(include isolated nodes)
    ...
    def attention_encoder(self, args, smiles, ob):
        with tf.variable_scope('attention_encoder', reuse=tf.AUTO_REUSE):
            ...
            importance_vec = importance_prob * emb_smiles  # b*120*8
            context_vec = tf.reduce_sum(importance_vec, axis=1, keepdims=True)  # b*1*8
            return context_vec
